...
let AccountBalanceService = (() => {
	...
	return class AccountBalanceService extends _classSuper {
		static {
			...
			__esDecorate(this, null, _quotas_decorators, {
				kind: "method",
				name: "quotas",
				static: false,
				private: false,
				access: {
					has: (obj) => "quotas" in obj,
					get: (obj) => obj.quotas
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
		...
		*/
		status() {
			...
			if (agents !== void 0) for (const agent of agents.list()) {
				const sessionId = agent.session.id;
				if (typeof sessionId === "string") seen.add(sessionId);
			}
			let tasks = 0;
			if (jobs !== void 0) {
				for (const job of jobs.list()) if (job.status === "running") tasks++;
			}
			return {
				sessions: seen.size,
				tasks
		...
		* `unit === 6 && number === 1` the weekly allowance. */
		async fetchGlm() {
			try {
				...
				if (key === void 0) return windowsRow("no-key", EMPTY_WINDOW, EMPTY_WINDOW);
				const limits = (await this.getJson(GLM_QUOTA_URL, key))?.data?.limits;
				let w5Raw;
				let weekRaw;
		...
		* window; string numbers and ISO reset timestamps are normalized. */
		async fetchKimi() {
			try {
				...
				let w5Raw;
				if (Array.isArray(body?.limits) && body.limits.length > 0) w5Raw = body.limits[0]?.detail;
		...
		async fetchOpenRouter() {
			try {
				...
				if (key === void 0) return balanceRow("no-key");
				const data = (await this.getJson(OPENROUTER_CREDITS_URL, key))?.data;
				if (data === void 0 || data === null) return balanceRow("no-data");
				return {
					mode: "balance",
					status: "ok",
					balance: Math.max(0, toNumber(data.total_credits) - toNumber(data.total_usage)),
					detail: "ok"
				};
			} catch {
		...
		async fetchDeepSeek() {
			try {
				...
				if (key === void 0) return balanceRow("no-key");
				const infos = (await this.getJson(DEEPSEEK_BALANCE_URL, key))?.balance_infos;
				if (!Array.isArray(infos) || infos.length === 0) return balanceRow("no-data");
				const first = infos[0];
				return {
					mode: "balance",
					status: "ok",
					balance: toNumber(first?.total_balance),
					detail: "ok"
				};
			} catch {
				return balanceRow("error");
...
//#endregion
export { AccountBalanceService, apply, name };
